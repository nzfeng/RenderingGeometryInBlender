...
\begin{itemize}
    ...
    \item All keyboard commands assume PC commands. The main difference vs. Mac is that \keys{\ctrl} becomes \keys{\cmd} (command) on Mac. I'm not sure what else changes. Maybe some keyboard shortcuts that involve keys like \keys{Alt}.
\end{itemize}
...
I save the vectors encoding rotation information in an OBJ file, with the vectors as vertex positions. A straightforward way to display a vector field is to have one vector per vertex or face, although for finely-tessellated meshes this will produce a vector field that is too densely sampled and makes for a poor visualization. A better approach is to use some method for \href{https://www.cs.ubc.ca/~rbridson/docs/bridson-siggraph07-poissondisk.pdf}{Poisson Disk Sampling}, which was used in the \href{https://www.cs.cmu.edu/~kmcrane/Projects/VectorHeatMethod/paper.pdf}{Vector Heat Method paper}.
...
\begin{enumerate}
    ...
    One way to do this is save yet another file containing the scalar function evaluated at each vector of your vector field, and import this data via nodes. Again, this is a non-optimal way to import attribute data; scripting would be a much better ``one-shot'' way to import data (you could simply write a Python script that reads any data you'd like, in any way you'd like, from any file(s).) 
    
    The following commands assume Blender 3.2.1. Go to the Geometry Shading editor where you have all the geometry nodes for your vector field set up. Add a \obj{Store Named Attribute} node, and a \obj{Realize Instances} node. (These nodes don't appear in Blender 3.0.0.) Feed the last of nodes controlling the instanced geometry into \obj{Store Named Attribute}. Input the scalar data you want to store on the instances into \obj{Store Named Attribute}, and set the attribute domain to ``Instance'', and set the data type appropriately (in this case, float.) Give a name to your attribute; here I've just named it ``col''. The next node, \obj{Realize Instances}, essentially converts instances to their own geometry, so that we can define attributes on them -- currently there is no direct way to store attributes on instanced geometry. Node configurations are shown in Fig.~\ref{fig:instances_attributes_node}.
    \begin{figure}[H]
        \centering
        \captionsetup{width=0.8\textwidth}
        \includegraphics[width=0.9\textwidth]{images/instances_attributes_node.png}
        \caption{Left: Adding the nodes that will allow us to define an attribute on instanced geometry. Right: The nodes used to extract the scalar field values at the instance samples; in this example, I've stored the values as the x-coordinate of vertex positions in a separate OBJ file.}
    ...
    \begin{figure}[H]
        \centering
        \captionsetup{width=0.8\textwidth}
        \includegraphics[width=\textwidth]{images/color_instances_luminance_nodes.png}
        \caption{The updated node configuration in the Shader editor. Probably a better way to do the inverse mapping (instead of the math nodes I used), is to simply use an \obj{Invert} node.}
...
Blender comes with a bunch of default materials and light sources. You can view these resources by navigating to the Blender application, right-clicking and selecting ``Show Package Contents'' (on Mac.) The directory \directory{Contents / Resources / [version] / datafiles / studiolights / world} contains some nice default lights that will illuminate the scene from all angles. In Linux, you also navigate to where your Blender application is located, and go to \directory{[version] / datafiles / studiolights}.

Typically I don't use environment lights; usually I find that adding a few area lights works better. (See Section~\ref{sec:DirectionalLighting})
...
Typically one adds a plane onto which the shadow from the object is cast. To add a plane, go to \menu{Add > Mesh > Plane}.
\begin{figure}[H]
    \centering
...
\end{figure}
Oftentimes, the shadow is initially too dark. The easiest way to remedy this is to change the alpha value of the ground plane: Go to the \menu{Shading} editor, select the plane, and set its alpha value to something less than 1.
...
Alternatively, you can add a ``shadow'' in a 2D graphics editor like Adobe Illustrator that appears physically correct. Creating a shadow in this manner will be easier with regards to controlling the extent of the shadow, its shape, its crispness, etc. However, the shadow will almost inevitably have a more cartoonish/cel-shaded aesthetic to it. This might fit exactly into your aesthetic goals for the figure -- or it might make your render look fake and funny, so you should exercise good judgement! % Sometimes physically-accurate shadows can be too complicated and distracting, and you desire a more streamlined figure that's a blend between realistically ray-traced \& a diagram/didactic feel.
...
\subsection{Accurate colormapping}

\section{Materials}
\label{sec:Materials}
I usually just adjust the Specular, Roughness, and Clearcoat paramters of the \obj{Principal BSDF} node in the Shader until I'm happy with it.
